  ...
  >
    <template v-if="type == 'password'">
      ...
      <div class="check-container remember-pwd">
        <t-checkbox style="color: #fff">记住账号</t-checkbox>
    ...
    <template v-else>
      <t-form-item name="phone">
        <t-input v-model="formData.phoneNumber" size="large" placeholder="请输入手机号码">
          <template #prefix-icon>
            <t-icon name="mobile" />
          </template>
        </t-input>
...
const form = ref<FormInstanceFunctions>(); // 验证字段是否合法
const formData = ref({ ...LoginInfo });
const showPsw = ref(false);

const [countDown, handleCounter] = useCounter();
const type = ref('password'); // 切换登录方式
